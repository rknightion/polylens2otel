...
var comments = map[string]string{
	...
	"phone.auth.from_lens_policy":             "Prefer credentials from the selected Lens policy; defaults to true. Set false to use only the configured password.",
	...
	for name, want := range outputs {
		...
		if *check {
			if err != nil || !bytes.Equal(got, want) {
				fatal(fmt.Errorf("generated configuration drift: %s (run just gen)", name))
			}
			continue
		}

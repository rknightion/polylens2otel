...
- **Never use `--notes` or `--plan` bare** — they silently replace the whole section and destroy another session's writes at exit 0. Use `--append-notes` and `--append-plan`. `.claude/hooks/backlog-guard.py` denies the bare forms; `.claude/hooks/backlog-guard_test.py` is its negative test.

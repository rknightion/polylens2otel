...
def _overview(builder: v2.Builder) -> dict:
    health = [
        _stat(builder, "Lens-connected phones", v2.prometheus(f"sum(polylens_device_connected{{{FLEET}}})", instant=True),
              description="Phones currently connected to Poly Lens."),
        _stat(builder, "Phone APIs healthy", v2.prometheus(f'sum(polyphone_api_state{{{FLEET},state="ok"}})', instant=True),
              description="Phones whose direct REST API is reachable, identified, and authenticated."),
        _stat(builder, "Registered lines", v2.prometheus(f"sum(polyphone_line_registered{{{FLEET}}})", instant=True),
              description="Currently registered logical phone lines."),
        _stat(builder, "Active calls", v2.prometheus(f"sum(polylens_device_active_calls{{{FLEET}}})", instant=True),
              description="Calls Poly Lens currently reports active."),
        _stat(builder, "Collectors healthy", v2.prometheus(f"sum(polylens2otel_collector_availability{{{COLLECTOR}}})", instant=True),
              description="Collector runs whose most recent execution succeeded."),
        _stat(builder, "HTTP errors (1h)", v2.prometheus(f"sum(increase(polylens2otel_http_4xx_total{{{TENANT}}}[1h])) + sum(increase(polylens2otel_http_5xx_total{{{TENANT}}}[1h]))", instant=True),
              description="Lens and phone client HTTP 4xx/5xx responses over the last hour.",
              threshold_config=v2.thresholds((None, "green"), (1, "red"))),
    ]
    fleet = [
        _ts(builder, "Fleet connectivity", [
            v2.prometheus(f"polylens_device_connected{{{FLEET}}}", "{{device_name}} Lens"),
            v2.prometheus(f'polyphone_api_state{{{FLEET},state="ok"}}', "{{device_name}} phone API"),
            v2.prometheus(f"polyphone_line_registered{{{FLEET}}}", "{{device_name}} line {{line}}"),
        ...
        _ts(builder, "Collector duration vs expected interval", [
            v2.prometheus(f"polylens2otel_collector_duration{{{COLLECTOR}}}", "{{collector_id}} duration"),
            v2.prometheus(f"polylens2otel_collector_expected_interval{{{COLLECTOR}}}", "{{collector_id}} interval"),
        ], unit="s", description="A collector approaching its interval cannot keep its schedule."),
    ]
    per_device = [
        _stat(builder, "Lens connected", v2.prometheus(f"polylens_device_connected{{{FLEET}}}", instant=True), unit="bool", mappings=BOOL_MAPPING),
        _stat(builder, "Phone API healthy", v2.prometheus(f'max(polyphone_api_state{{{FLEET},state="ok"}})', instant=True), unit="bool", mappings=BOOL_MAPPING),
        _stat(builder, "Lines registered", v2.prometheus(f"sum(polyphone_line_registered{{{FLEET}}})", instant=True)),
        _stat(builder, "Uptime", v2.prometheus(f"polyphone_uptime_seconds{{{FLEET}}}", instant=True), unit="s"),
        _stat(builder, "Firmware current", v2.prometheus(f"polylens_device_firmware_current{{{FLEET}}}", instant=True), unit="bool", mappings=BOOL_MAPPING),
        _stat(builder, "Active calls", v2.prometheus(f"sum(polylens_device_active_calls{{{FLEET}}})", instant=True)),
    ]
    return v2.tab("Overview", [
        v2.row("Fleet health", health),
        v2.row("Health over time", fleet),
        v2.row("Per-device health", per_device, repeat="device"),
    ])
...
def _lens(builder: v2.Builder) -> dict:
    return v2.tab("Lens fleet", [
        v2.row("Connection and recency", [
            _ts(builder, "Lens connection state", [v2.prometheus(f"polylens_device_connected{{{FLEET}}}", "{{device_name}}")], unit="bool"),
            _ts(builder, "Time since Lens detected phone", [v2.prometheus(f"polylens_device_last_detected_seconds{{{FLEET}}}", "{{device_name}}")], unit="s"),
            _ts(builder, "Time since configuration request", [v2.prometheus(f"polylens_device_last_config_request_seconds{{{FLEET}}}", "{{device_name}}")], unit="s"),
            _ts(builder, "Device stream", [v2.prometheus(f"polylens_stream_connected{{{TENANT}}}", "stream")], unit="bool"),
        ]),
        v2.row("Firmware and calls", [
            _table(builder, "Firmware inventory", [
                v2.prometheus(f"polylens_device_firmware_info{{{FLEET}}}", instant=True, fmt="table"),
                v2.prometheus(f"polylens_device_firmware_current{{{FLEET}}}", instant=True, fmt="table"),
            ], description="Installed version/build and whether Lens reports it current."),
            _ts(builder, "Active calls by state", [v2.prometheus(f"polylens_device_active_calls{{{FLEET}}}", "{{device_name}} {{state}}")], description="Active-call counts supplied by Lens."),
        ]),
    ])
...
def _phone(builder: v2.Builder) -> dict:
    return v2.tab("Phone REST", [
        v2.row("Reachability and registration", [
            _table(builder, "Phone API state", [v2.prometheus(f"polyphone_api_state{{{FLEET}}}", instant=True, fmt="table")],
                   description="Exactly one of ok, api_disabled, auth_failed, or unreachable is 1 per phone."),
            _ts(builder, "Line registration", [v2.prometheus(f"polyphone_line_registered{{{FLEET}}}", "{{device_name}} line {{line}}")], unit="bool"),
            _ts(builder, "Logical lines", [v2.prometheus(f"polyphone_lines_total{{{FLEET}}}", "{{device_name}}")]),
            _ts(builder, "Phone uptime", [v2.prometheus(f"polyphone_uptime_seconds{{{FLEET}}}", "{{device_name}}")], unit="s"),
        ]),
        v2.row("Network and configuration", [
            _ts(builder, "Packet rate", [v2.prometheus(f"rate(polyphone_network_packets_total{{{FLEET}}}[$__rate_interval])", "{{device_name}} {{direction}}")], unit="pps"),
            _table(builder, "Network inventory", [v2.prometheus(f"polyphone_network_info{{{FLEET}}}", instant=True, fmt="table")],
                   description="DHCP, gateway, subnet, boot-server, and DHCP-server metadata."),
            _table(builder, "Configuration source", [v2.prometheus(f"polyphone_config_param_source{{{FLEET}}}", instant=True, fmt="table")],
...
def _calls(builder: v2.Builder) -> dict:
    ...
    return v2.tab("Calls & logs", [
        v2.row("Call activity", [
            _ts(builder, "Phone call-log records", [v2.prometheus(f"rate(polyphone_calls_total{{{FLEET}}}[$__rate_interval])", "{{device_name}} {{direction}}")], unit="ops"),
            _ts(builder, "Lens CDR records", [v2.loki('{service_name="polylens2otel"} | event_name="polylens.cdr" | tenant_id=~"$tenant" | unwrap duration_seconds | __error__=""')], unit="s",
                description="Durations of Lens CDR rows. Empty means no calls in the selected range."),
        ]),
...
def _self_o11y(builder: v2.Builder) -> dict:
    return v2.tab("Self-o11y", [
        v2.row("Build and scheduling", [
            _table(builder, "Running build", [v2.prometheus(f"polylens2otel_build_info{{{TENANT}}}", instant=True, fmt="table")]),
            _ts(builder, "Collector availability", [v2.prometheus(f"polylens2otel_collector_availability{{{COLLECTOR}}}", "{{collector_id}}")], unit="bool"),
            _ts(builder, "Collector duration", [v2.prometheus(f"polylens2otel_collector_duration{{{COLLECTOR}}}", "{{collector_id}}")], unit="s"),
            _ts(builder, "Expected interval", [v2.prometheus(f"polylens2otel_collector_expected_interval{{{COLLECTOR}}}", "{{collector_id}}")], unit="s"),
        ]),
        v2.row("Upstream APIs and authentication", [
            _ts(builder, "HTTP request duration", [v2.prometheus(f"polylens2otel_http_client_request_duration{{{TENANT}}}", "{{source}} {{device_name}}")], unit="s"),
            _ts(builder, "HTTP client errors", [
                v2.prometheus(f"rate(polylens2otel_http_4xx_total{{{TENANT}}}[$__rate_interval])", "4xx {{source}}"),
                v2.prometheus(f"rate(polylens2otel_http_5xx_total{{{TENANT}}}[$__rate_interval])", "5xx {{source}}"),
            ], unit="ops"),
            _ts(builder, "Unexpected API shapes", [v2.prometheus(f"polylens2otel_api_unexpected{{{TENANT}}}", "{{source}} {{device_name}}")]),
            _ts(builder, "Lens token lifecycle", [
                v2.prometheus(f"rate(polylens2otel_auth_token_refresh_total{{{TENANT}}}[$__rate_interval])", "refreshes/s"),
                v2.prometheus(f"polylens2otel_auth_token_expires_seconds{{{TENANT}}}", "expires in"),
            ], unit="s"),
        ]),
        v2.row("Streaming and ingestion", [
            _ts(builder, "Stream reconnects", [v2.prometheus(f"rate(polylens2otel_stream_reconnects_total{{{TENANT}}}[$__rate_interval])", "reconnects/s")], unit="ops"),
            _ts(builder, "Stream messages", [v2.prometheus(f"rate(polylens2otel_stream_messages_total{{{TENANT}}}[$__rate_interval])", "messages/s")], unit="ops"),
            _ts(builder, "Time since stream message", [v2.prometheus(f"polylens2otel_stream_last_message_seconds{{{TENANT}}}", "age")], unit="s"),
            _ts(builder, "Ingest accounting", [
                v2.prometheus(f"rate(polylens2otel_ingest_emitted_points_total{{{COLLECTOR}}}[$__rate_interval])", "{{collector_id}} emitted"),
                v2.prometheus(f"rate(polylens2otel_ingest_source_records_total{{{COLLECTOR}}}[$__rate_interval])", "{{collector_id}} source"),
            ], unit="ops"),
            _ts(builder, "Checkpoint persistence errors", [v2.prometheus(f"rate(polylens2otel_checkpoint_persist_errors_total{{{COLLECTOR}}}[$__rate_interval])", "{{collector_id}}")], unit="ops"),
        ]),
    ])
...
def _traces(builder: v2.Builder) -> dict:
    ...
    return v2.tab("Traces", [
        v2.row("Trace search", [
            builder.panel("Collector-run traces", "traces", [v2.tempo(f'{{ resource.service.name = "polylens2otel" && name =~ "collector\\..+" && {tenant_trace} }}')],
                          width=24, height=10, description="One trace per scheduled collector run, with child HTTP calls."),
            builder.panel("Outbound HTTP spans", "traces", [v2.tempo(f'{{ resource.service.name = "polylens2otel" && name = "http.client.request" && {tenant_trace} }}')],
                          width=24, height=10, description="Instrumented Lens and phone HTTP calls."),
        ]),
        v2.row("Trace-derived health", [
            _ts(builder, "Collector p95 duration", [v2.tempo(f'{{ resource.service.name = "polylens2otel" && name =~ "collector\\..+" && {tenant_trace} }} | quantile_over_time(duration, 0.95) by (name)')], unit="s"),
            ...
            _ts(builder, "Collector run rate", [v2.tempo(f'{{ resource.service.name = "polylens2otel" && name =~ "collector\\..+" && {tenant_trace} }} | rate() by (name)')], unit="ops"),
